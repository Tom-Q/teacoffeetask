...
def train_supervised(model, num_episodes):
    ...
    for episode in range(num_episodes):
        ...
        with tf.GradientTape() as tape:
            ...
            model.context = np.random.uniform(0.01, 0.99, (1, model.size_hidden))
...
def train_pnas():
    ...
    for episode in range(num_episodes):
        ...
        with tf.GradientTape() as tape:
            ...
            ratios = evaluate([tchoices], [targets])
            # Train model, record loss.
    ...
    for sequence in seqs:
        ...
        with tf.GradientTape() as tape:
            ...
            for i in range(len(targets)):
                ...
                #env.do_action(targets[i])  # Transition the MDP according to the *target* action, not the chosen action!
